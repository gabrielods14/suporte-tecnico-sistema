...
class PendingTicketsPage(BasePage):
    ...
    def _do_load_tickets(self):
        """Faz carregamento"""
        try:
            # Colaboradores (permissão 1) veem apenas seus próprios chamados
            # Técnicos de suporte (permissão 2) e Administradores (permissão 3) veem todos
            filters = {}
            permissao = self.user_info.get('permissao', 1)
            if permissao == 1:  # Colaborador - apenas seus chamados
                user_id = self.user_info.get('id')
                if user_id:
                    filters['solicitanteId'] = int(user_id)
            
            api_tickets = TicketService.get_tickets(filters)
            
            if api_tickets:
                mapped = []
                for item in api_tickets:
                    try:
                        abertura = datetime.fromisoformat(item.get('dataAbertura', datetime.now().isoformat()).replace('Z', '+00:00'))
                    except:
                        abertura = datetime.now()
                    
                    if item.get('dataLimite'):
                        try:
                            limite = datetime.fromisoformat(item.get('dataLimite').replace('Z', '+00:00'))
                        except:
                            limite = abertura + timedelta(days=7)
                    else:
                        limite = abertura + timedelta(days=7)
                    ...
                    if status in [1, 2, 4]:  # Aberto, EmAndamento, Resolvido (como na web)
                        mapped.append({
                            'id': item.get('id'),
                            'codigo': str(item.get('id', 0)).zfill(6),
                            'titulo': item.get('titulo', ''),
                            'prioridade': prioridade_text,
                            'dataLimite': limite.isoformat(),
                            'status': status
                        })
    ...
    def _apply_filters_and_sort(self):
        ...
        def sort_key(ticket):
            ...
            elif self.sort_by == 'dataLimite':
                try:
                    return datetime.fromisoformat(ticket['dataLimite'].replace('Z', '+00:00'))
                except:
                    return datetime.now()
            return 0
    ...
    def _calculate_days_to_expire(self, data_limite):
        ...
        try:
            limite = datetime.fromisoformat(data_limite.replace('Z', '+00:00'))
            hoje = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
            limite = limite.replace(hour=0, minute=0, second=0, microsecond=0)
            
            diff = (limite - hoje).days
            
            if diff < 0:
                return f"Vencido há {abs(diff)} dias"
            elif diff == 0:
                return "Vence hoje"
            else:
